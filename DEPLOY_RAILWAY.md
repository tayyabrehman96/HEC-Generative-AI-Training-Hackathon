...
| **Timeout** / `aborted due to timeout` | Large vision models (e.g. `qwen3.5-122b`) may need **15–40+ minutes**. Default proxy wait is **20 min**; set **`REGOLO_FETCH_TIMEOUT_MS`** (e.g. `1800000` for 30 min, max **45 min** / `2700000`) on Railway if logs show `Regolo HTTPS timeout`. |
